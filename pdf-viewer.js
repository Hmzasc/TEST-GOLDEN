...
(function () {
    ...
    let currentDownloadName = 'ملف.pdf';
    ...
    async function openPDFViewer(url, title, filename) {
        const e = els();
        if (!e.modal) {
        ...
        // نخزّن الرابط والاسم الحقيقي للاستخدام عند الضغط على "تنزيل"
        // (يتم التنزيل الفعلي عبر download.js لضمان نجاحه مع أي رابط خارجي
        // وحفظ الاسم الصحيح وإضافة العلامة المائية)
        currentDownloadUrl = url;
        currentDownloadName = filename || ((title || 'ملف') + '.pdf');
        currentPageNum = 1;
        currentScale = 1.2;
        currentRotation = 0;
    ...
    function bindEvents() {
        ...
        // زر التنزيل داخل نافذة العرض: يجب أن يمر عبر download.js
        // (تنزيل حقيقي عبر Blob) وليس رابطاً مباشراً — الروابط المباشرة
        // لملفات من نطاق مختلف (مثل Cloudinary) يتجاهلها المتصفح
        // ويفتح الملف بدل تنزيله.
        if (e.downloadLink) {
            e.downloadLink.addEventListener('click', (ev) => {
                ev.preventDefault();
                if (typeof downloadWithWatermark === 'function') {
                    downloadWithWatermark(currentDownloadUrl, currentDownloadName);
                } else {
